...
def metric_plot(fig, ax, metric, lon, lat, vmin, vmax, title, cmap):
    ...
    ix.hist(
        metric, bins=20, weights=np.zeros_like(metric) + 1 / len(metric), filled=True, alpha=0.3, c='red5'
    )

    cax = fig.add_axes([0.58, 0.25, 0.2, 0.03])
    cbar = plt.colorbar(m, cax=cax, orientation='horizontal', pad=0.06, extend='both', aspect=30)
...
for df, file_name in zip(df_list, file_list):
    for column, vmin, vmax, title in zip(columns, vmins, vmaxs, titles):
        ...
        fig.savefig(f'D:\\ET\\Figures\\{file_name}_{column[:3]}.png', bbox_inches='tight', dpi=400)
