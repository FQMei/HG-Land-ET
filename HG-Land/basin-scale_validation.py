...
mrb_id = pd.read_csv(r'D:\ET\DataCollect\Runoff\GRDC\mrb_sel_shp\mrb_info.csv', index_col=0)
# %%
# CLASS
...
for index in mrb_id.index:
    et = clas.where(mrb_mask.region == index).spatial.average('hfls', axis=['X', 'Y'])['hfls'].compute()
...
# %%
# HG-Land
hget = xc.open_dataset(r'D:\ET\MLResults\HG-Land_ET_1982-2018_MO_ensmn.nc')
...
for index in mrb_id.index:
    et = hget_sel.where(mrb_mask.region == index).spatial.average('LE', axis=['X', 'Y'])['LE']
    df = (et * 0.035 * days).to_dataframe()
...
for index in mrb_id.index:
    et = glma_sel.where(mrb_mask.region == index).spatial.average('E', axis=['X', 'Y'])['E']
    df = et.to_dataframe()
...
for index in mrb_id.index:
    et = flux_sel.where(mrb_mask.region == index).spatial.average('LE', axis=['X', 'Y'])['LE']
