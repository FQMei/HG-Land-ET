...
# %%
hget_path = r"D:\ET\MLResults\HG-Land_ET_1982-2018_MO.nc"
...
# %%
hget_ann = (
    hget.LE.sel(time=slice("1982-01", "2016-12")).mean(dim="time", skipna=True) * 12.876
)
...
# %%
norm_spatial = colors.TwoSlopeNorm(vmin=0, vmax=1600)
norm_diff = colors.Normalize(vmin=-1000, vmax=1000)

color_list = ["#ef767a", "#456990", "#49beaa"]
labels = ["HG-Land", "FLUXCOM", "GLEAM"]
da = [hget_ann, hget_ann_mask - flux_ann, hget_ann_mask - gleam_ann_mask]
...
conf = Spatial_map(ax1, da=da[0], norm=norm_spatial, cmap="terrain_r")
...
cb2.ax.tick_params(which="both", direction="out", width=0.5, length=1)
