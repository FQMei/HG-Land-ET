...
# %%
feature = ['tmp', 'pre', 'vap', 'pet', 'wet', 'frs', 'rad', 'wnd', 'tsk', 'prs', 'fAPAR', 'LAI']
# %%
...
def train_Deepforest(x_train, y_train, estm=2, trees=100):
    Deepforest = CascadeForestRegressor(
        backend='sklearn', n_estimators=estm, n_trees=trees, partial_mode=True, random_state=2
    )
...
def get_test_predict(model, x_test, y_info, y_test):
    ...
    df_test = pd.DataFrame(data={'EstimateLE': y_test, 'predict (W/m2)': y_predict.ravel()})
...
# %%
pft_as_test = 'MF'  # 'CRO', 'CSH', 'DBF', 'EBF', 'ENF', 'GRA', 'OSH', 'SAV', 'WET', 'WSA'
...
df_train, df_test, X_train, X_test, Y_train, Y_test, Y_info = get_features(train_file, test_file)
# %%
...
# %%
# SHAP
explainer = shap.Explainer(dpmodel.predict, X_train)

x = X_train.sample(100)
shap_values = explainer(x)

shap.plots.bar(shap_values, max_display=12, show=False)
plt.savefig(f'D:\\ET\\Figures\\shap_bar_{pft_as_test}.png', dpi=400, bbox_inches='tight')
plt.show()
# %%
# ALE
xlabels = [
...
fig, axs = pplt.subplots(ncols=4, nrows=3, figsize=(8, 5), dpi=400, sharey='labels', sharex=False)

axs.format(abc='(a)', abcloc='ul')

for i in range(12):
    ale.ale(
        X_train, dpmodel, [feature[i]], xlabels[i], grid_size=50, include_CI=True, C=0.95, ax=axs[i], fig=fig
    )

    fig.savefig(f'D:\\ET\\Figures\\ale_{pft_as_test}.png', bbox_inches='tight', dpi=400)
# %%
model_dir = f'D:\\ET\\MLResults\\Deepf_model_{pft_as_test}'
dpmodel.save(model_dir)
# %%
