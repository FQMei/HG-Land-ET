...
koppen = np.genfromtxt(
    r"F:\Koppen Climate map\data\koppen_1901-2010.tsv", dtype=None, names=True
)

df = pd.DataFrame(koppen)
df.rename(
    columns={"p1901_2010": "koppen", "longitude": "lon", "latitude": "lat"},
    inplace=True,
)
# %%
...
# hg-land
hgld = xr.open_dataset(r"D:\ET\MLResults\HG-Land_ET_product\HG-Land_ET_1982-2018_MO.nc")
...
for ds, file_name in zip(ds_list, name_list):
    ...
    for i in range(4):
        ...
        arr_koppen = (
            da_koppen.mean(dim=["lon", "lat"], skipna=True)
            .groupby("time.month")
            .mean("time")
        )
...
fig, axs = pplt.subplots(
    ncols=2, nrows=2, figsize=(6, 5), sharex=1, sharey=1, span=True
)
